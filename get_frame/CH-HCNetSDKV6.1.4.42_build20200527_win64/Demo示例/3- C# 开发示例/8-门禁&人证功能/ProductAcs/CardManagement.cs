...
namespace AcsDemo.ProductAcs
{
    public partial class CardManagement : Form
    {
        ...
        private void ProcessSetGatewayCardCallback(uint dwType, IntPtr lpBuffer, uint dwBufLen, IntPtr pUserData)
        {
            ...
            uint dwStatus = (uint)Marshal.ReadInt64(lpBuffer);
            ...
            if (dwStatus == (uint)CHCNetSDK.NET_SDK_CALLBACK_STATUS_NORMAL.NET_SDK_CALLBACK_STATUS_PROCESSING)
            {
                ...
                byte[] bRawData = new byte[36];//4字节状态 + 32字节卡号
                Marshal.Copy(lpBuffer, bRawData, 0, 36);//将非托管内存指针数据复制到数组中

                byte[] byCardNo = new byte[32];//32字节卡号
                Array.Copy(bRawData, 4, byCardNo, 0, 32);
                string strCardNo = Encoding.ASCII.GetString(byCardNo).TrimEnd('\0');

                string strTemp = null; 
                if (m_bSendOne)
                {
                    strTemp = string.Format("Send SUCC,CardNO:{0}", strCardNo);
                    m_bSendOne = false;
                }
                else
                {
            ...
            else if (dwStatus == (uint)CHCNetSDK.NET_SDK_CALLBACK_STATUS_NORMAL.NET_SDK_CALLBACK_STATUS_FAILED)
            {
                ...
                if (m_iSelectIndex!=-1)
                {
                    strTemp = string.Format("Send FAILED,CardNO:{0},Error code:{1}", strCardNo, errorCode);
                    listItem.SubItems.Add(strTemp);
                    this.AddList(listViewMessage, listItem, true);
                }
                //next
        ...
        private void UpdateList(int iInsertIndex, string strCardNo, CHCNetSDK.NET_DVR_CARD_CFG_V50 struCardInfo)
        {
            ...
            lvItem.SubItems.Add(struCardInfo.dwEmployeeNo.ToString());

            if (m_iEncodeType == 6)
            {
                lvItem.SubItems.Add(System.Text.Encoding.UTF8.GetString(struCardInfo.byName));
            }
            else
            {
                lvItem.SubItems.Add(System.Text.Encoding.Default.GetString(struCardInfo.byName));
            }
        ...
        private void buttonBatchAdd_Click(object sender, EventArgs e)
        {
            ...
            batchAdd.SetEncodeType(m_iEncodeType);
        ...
        private void buttonDeleteCard_Click(object sender, EventArgs e)
        {
            ...
            if (m_BSendSel)
            {
                ...
                //2019/06/27 Murphy Zhang Start
                m_struSelSendCardCfg.dwModifyParamType |= 0x1;
                //2019/06/27 Murphy Zhang End
                SendCardData(m_struSelSendCardCfg);
                m_BSendSel = false;
                m_struSelSendCardCfg = new CHCNetSDK.NET_DVR_CARD_CFG_V50();
