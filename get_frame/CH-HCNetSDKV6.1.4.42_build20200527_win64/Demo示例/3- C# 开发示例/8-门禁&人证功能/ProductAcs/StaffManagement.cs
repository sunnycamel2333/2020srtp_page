...
namespace AcsDemo.ProductAcs
{
    public partial class StaffManagement : Form
    {
        ...
        public Int32 m_iUserInfoSettingHandle = -1;
        ...
        private CHCNetSDK.RemoteConfigCallback g_fUserInfoSettingCallback = null;
        ...
        public StaffManagement()
        {
            ...
            checkBoxCheckUser.Checked = true;
            UpdateListRightPlan();
        ...
        private bool SendUserInfo()
        {
            ...
            if (listViewStaffManage.Items[m_iUserInfoRecordIndex].SubItems[20].Text == "True")
            {
                JsonUserInfoRecord.UserInfo.checkUser = true;
            }
            else if (listViewStaffManage.Items[m_iUserInfoRecordIndex].SubItems[20].Text == "False")
            {
                JsonUserInfoRecord.UserInfo.checkUser = false;
            }
        ...
        private void buttonBatchAdding_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < iAddUserNumber; i++)
            {
                ...
                listItem.SubItems.Add(checkBoxCheckUser.Checked.ToString());
        ...
        private void btnSetting_Click(object sender, EventArgs e)
        {
            ...
            g_fUserInfoSettingCallback = new CHCNetSDK.RemoteConfigCallback(ProcessUserInfoRecordCallback);
            string sURL = "PUT /ISAPI/AccessControl/UserInfo/SetUp?format=json";
            IntPtr ptrURL = Marshal.StringToHGlobalAnsi(sURL);
            m_lUserInfoRecordHandle = CHCNetSDK.NET_DVR_StartRemoteConfig(m_lUserID, CHCNetSDK.NET_DVR_JSON_CONFIG, ptrURL, sURL.Length, g_fUserInfoSettingCallback, this.Handle);
            if (-1 == m_lUserInfoRecordHandle)
            {
                ListViewItem listItem = new ListViewItem();
                listItem.Text = "Fail";
                string strTemp = string.Format("User Info Set Fail,Error Code={0}", CHCNetSDK.NET_DVR_GetLastError());
                listItem.SubItems.Add(strTemp);
                this.AddList(listViewMessage, listItem);
                Marshal.FreeHGlobal(ptrURL);
                return;
            ...
            else
            {
                ListViewItem listItem = new ListViewItem();
                listItem.Text = "Succ";
                listItem.SubItems.Add("User Info Set Success");
                this.AddList(listViewMessage, listItem);
            }
            Marshal.FreeHGlobal(ptrURL);
            if (!SendUserInfo())
        ...
        private void buttonUpdateList_Click(object sender, EventArgs e)
        {
            if (listViewStaffManage.SelectedItems.Count > 0)
            {
                ...
                listViewStaffManage.SelectedItems[0].SubItems[20].Text = checkBoxCheckUser.Checked.ToString();
            }
        }
        ...
        private void listViewStaffManage_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            if (listViewStaffManage.SelectedItems.Count > 0)
            {
                ...
                if (e.Item.SubItems[20].Text == "True")
                {
                    checkBoxCheckUser.Checked = true;
                }
                else if (e.Item.SubItems[20].Text == "False")
                {
                    checkBoxCheckUser.Checked = false;
                }
            }
        }
            ...
            switch (m.Msg)
            {
                ...
                case 1006:
                    for (int i = 0; i < m_JsonUserInfoSearch.UserInfoSearch.numOfMatches; i++)
                    {
                        ...
                        if (m_JsonUserInfoSearch.UserInfoSearch.UserInfo[i].checkUser == true)
                        {
                            listItem.SubItems.Add("True");
                        }
                        else
