...
namespace AcsDemo.ProductAcs
{
    public partial class EventCardLinkage : Form
    {
        ...
        private void comboBoxMajorEventType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboBoxMajorEventType.SelectedIndex == 0)
            {
                SubEventTypeDev();
                //comboBoxMinorEventType.Items.Clear();
                //comboBoxMinorEventType.Items.AddRange(new object[]{"EVENT_ACS_HOST_ANTI_DISMANTLE = 0",
                //"EVENT_ACS_OFFLINE_ECENT_NEARLY_FULL", 
                //"EVENT_ACS_NET_BROKEN,EVENT_ACS_NET_RESUME", 
                //"EVENT_ACS_LOW_BATTERY",
                //"EVENT_ACS_BATTERY_RESUME"});
                comboBoxMinorEventType.SelectedIndex = 0;
            }
            if (comboBoxMajorEventType.SelectedIndex == 1)
            {
                SubEventTypeAlarm();
                //comboBoxMinorEventType.Items.Clear();
                //comboBoxMinorEventType.Items.AddRange(new object[]{"EVENT_ACS_ALARMIN_SHORT_CIRCUIT = 0",
            ...
            if (comboBoxMajorEventType.SelectedIndex == 2)
            {
                SubEventTypeDoor();
                comboBoxMinorEventType.SelectedIndex = 0;
            }
            if (comboBoxMajorEventType.SelectedIndex == 3)
            {
                SubEventTypeCardReader();
                comboBoxMinorEventType.SelectedIndex = 0;
            }
