...
namespace AcsDemo.ProductAcs
{
    public partial class CardManagementByStaffCenter : Form
    {
        ...
        private CHCNetSDK.RemoteConfigCallback g_fCardInfoSettingCallback = null;
        ...
        public CardManagementByStaffCenter()
        {
            ...
            checkBoxcheckCard.Checked = true;
            checkBoxCheckEmployeeNo.Checked = true;
        ...
        private bool SendCardInfo()
        {
            ...
            if(checkBoxDelCard.CheckState==CheckState.Checked)
            {
                JsonCardInfoRecord.CardInfo.deleteCard = true;
            }
            else
            {
                JsonCardInfoRecord.CardInfo.deleteCard = false;
            }

            if(checkBoxcheckCard.CheckState==CheckState.Checked)
            {
                JsonCardInfoRecord.CardInfo.checkCardNo = true;
            }
            else
            {
                JsonCardInfoRecord.CardInfo.checkCardNo = false;
            }

            if (checkBoxCheckEmployeeNo.CheckState == CheckState.Checked)
            {
                JsonCardInfoRecord.CardInfo.checkEmployeeNo = true;
            }
            else
            {
                JsonCardInfoRecord.CardInfo.checkEmployeeNo = false;
            }

            string strCardInfoRecord = JsonConvert.SerializeObject(JsonCardInfoRecord, Formatting.Indented,
        ...
        private void btnSetting_Click(object sender, EventArgs e)
        {
            ...
            g_fCardInfoSettingCallback = new CHCNetSDK.RemoteConfigCallback(ProcessCardInfoRecordCallback);
            string sURL = "PUT /ISAPI/AccessControl/CardInfo/SetUp?format=json";
            IntPtr ptrURL = Marshal.StringToHGlobalAnsi(sURL);

            m_lCardInfoRecordHandle = CHCNetSDK.NET_DVR_StartRemoteConfig(m_lUserID, CHCNetSDK.NET_DVR_JSON_CONFIG, ptrURL, sURL.Length, g_fCardInfoSettingCallback, this.Handle);
            if (-1 == m_lCardInfoRecordHandle)
            {
                ListViewItem listItem = new ListViewItem();
                listItem.Text = "Fail";
                string strTemp = string.Format("Card Info Set Fail,Error Code={0}", CHCNetSDK.NET_DVR_GetLastError());
                listItem.SubItems.Add(strTemp);
                this.AddList(listViewMessage, listItem);
                Marshal.FreeHGlobal(ptrURL);
                return;
            ...
            else
            {
                ListViewItem listItem = new ListViewItem();
                listItem.Text = "Succ";
                listItem.SubItems.Add("Card Info Set Success");
                this.AddList(listViewMessage, listItem);
            }
            Marshal.FreeHGlobal(ptrURL);
            if (!SendCardInfo())
