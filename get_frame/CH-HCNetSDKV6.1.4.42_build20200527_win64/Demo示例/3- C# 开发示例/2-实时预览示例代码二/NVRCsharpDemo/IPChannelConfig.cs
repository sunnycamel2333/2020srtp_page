...
namespace NVRCsharpDemo
{
    public partial class IPChannelConfig : Form
    {
        ...
        private void btnSet_Click(object sender, EventArgs e)
        {
            ...
            m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].struIP.sIpV4 = new byte[16];
            byIpV4.CopyTo(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].struIP.sIpV4, 0);
            ...
            m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].sUserName = new byte[32];
            byUserName.CopyTo(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].sUserName, 0);

            //前端设备密码
            byte[] byPsw = System.Text.Encoding.Default.GetBytes(textBoxPsw.Text);
            m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].sPassword = new byte[16];
            byPsw.CopyTo(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].sPassword, 0);
        ...
        private void IPChannelConfig_Load(object sender, EventArgs e)
        {
            ...
            textBoxIPAddr.Text = System.Text.Encoding.UTF8.GetString(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].struIP.sIpV4);//前端设备IP地址
            textBoxPort.Text = Convert.ToString(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].wDVRPort);//前端设备端口
            textBoxUserName.Text = System.Text.Encoding.UTF8.GetString(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].sUserName);//前端设备用户名
            textBoxPsw.Text = System.Text.Encoding.UTF8.GetString(m_struIPParaCfgV40.struIPDevInfo[iIPDevIndex].sPassword);//前端设备密码
            for (i = 0; i < (int)m_struProtoList.dwProtoNum; i++)
            {
                //设备（厂家）协议类型,通过NET_DVR_GetIPCProtoList获取
