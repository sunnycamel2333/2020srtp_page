...
void CDlgPdcResult::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_AGEGROUP, m_bAgeGroup);
...
BOOL CDlgPdcResult::OnInitDialog() 
{
    ...
    g_StringLanType(szLanTemp, "性别", "Gender");
    ...
    g_StringLanType(szLanTemp, "年龄段", "AgeGroup");
...
void CDlgPdcResult::AddPdcResultLst(LPNET_DVR_PDC_RESULT lpInter)
{
    ...
    //年龄段人数统计
    memset(szStr, 0, sizeof(szStr));
    sprintf(szStr, "Unknown[%d]Child[%d]Young[%d]Middle[%d]Old[%d]Infant[%d]Kid[%d]Teenager[%d]Prime[%d]MiddleAged[%d]",
        lpInter->dwAgeGroupUnknown, lpInter->dwChild, lpInter->dwYoung, lpInter->dwMiddle, lpInter->dwOld,
        lpInter->dwInfant, lpInter->dwKid, lpInter->dwTeenager, lpInter->dwPrime, lpInter->dwMiddleAged);
...
void CDlgPdcResult::OnBtnGetPdcResult() 
{
    ...
    if (m_bAgeGroup)
    {
        m_struPdcResultCond.byAgeGroup = 0xfe;
    }
    if (m_bGender)
    {
