    ...
    {	
        ...
        {	
            ...
            else if (struSearchRet.wPicType == 1 || struSearchRet.wPicType == 2)
            {
                ...
                struSearchRet.wPicType == 18 || struSearchRet.wPicType == 19)
            {
...
BOOL CDlgSmartSearchPicture::CheckInitParam()
{
    ...
    g_StringLanType(szLan, "离岗", "LeavePosition");
    m_comBehaviorSearchType.AddString(szLan);

    g_StringLanType(szLan, "人数变化事件", "PeopleNumChange");
    m_comBehaviorSearchType.AddString(szLan);

    g_StringLanType(szLan, "剧烈运动", "ViolentMotion");
    m_comBehaviorSearchType.AddString(szLan);

    g_StringLanType(szLan, "奔跑检测", "Running");
    m_comBehaviorSearchType.AddString(szLan);

    g_StringLanType(szLan, "倒地检测", "FailDown");
    m_comBehaviorSearchType.AddString(szLan);

    g_StringLanType(szLan, "滞留检测", "Retention");
    m_comBehaviorSearchType.AddString(szLan);
