...
void CDlgStreamSrcCfg::OnButtonGetStreamSrc() 
{
    ...
    {        
        ...
        AddStatusLog(iDeviceIndex, OPERATION_FAIL_T, dwStatus,"Fail to set stream source");
        AfxMessageBox(szLan);
        return;
    }
    else
...
void CDlgStreamSrcCfg::AddStatusLog(int iDeviceIndex, int iLogType, DWORD dwStatus, const char* format, ...)
{
    //当接口返回正确，返回参数状态错误时进入该分支
    CTime  cTime = CTime::GetCurrentTime();
    int iLen = 0;
    char szTime[64] = { 0 };
    char szLogType[32] = "FAIL";
    char szLogInfo[1024 * 5] = "Fail to get stream source";
    char szDevInfo[256] = { 0 };
    char szErrInfo[256] = { 0 };
    char szLog[1024 * 5] = { 0 };

    sprintf(szErrInfo, "err[%d:%s]", dwStatus, "Parameter Error");
    ...
    try
    {
        ...
        pLogInfo->iLogType = OPERATION_FAIL_T;
