...
ON_BN_CLICKED(IDC_BTN_MATERIAL_THUMBNAIL_DOWNLOAD, &CDlgInfoDiffusionMaterial::OnBnClickedBtnMaterialThumbnailDownload)
END_MESSAGE_MAP()
...
void CDlgInfoDiffusionMaterial::OnBtnMaterialUpload() 
{
    ...
    memset(m_struMaterial.szMaterialType, 0, sizeof(m_struMaterial.szMaterialType));
    memset(m_struMaterial.szMaterialName, 0, sizeof(m_struMaterial.szMaterialName));
    memset(m_struMaterial.szMaterialRemarks, 0, sizeof(m_struMaterial.szMaterialRemarks));
    ...
    memset(m_struMaterial.struStaticMaterial.szStaticMaterialType, 0, sizeof(m_struMaterial.szUploadUser));
	...
	if (0 == strcmp(strMaterialType, "static")) //静态素材
	{
		...
		{			
			...
			CString strStaticMaterialFormat;
			m_cmbStaticMaterialFormat.GetLBText(nCurSel, strStaticMaterialFormat);
            memset(m_struMaterial.struStaticMaterial.szStaticMaterialFormat, 0, sizeof(m_struMaterial.struStaticMaterial.szStaticMaterialFormat));
	...
	else if (0 == strcmp(strMaterialType, "dynamic")) //动态素材
	{
        ...
        memset(m_struMaterial.struDynamicMaterial.szDynamicMaterialType, 0, sizeof(m_struMaterial.struDynamicMaterial.szDynamicMaterialType));
		...
		{
            memset(m_struMaterial.struDynamicMaterial.szRssUrl, 0, sizeof(m_struMaterial.struDynamicMaterial.szRssUrl));
...
//************************************
// Method:    OnBnClickedBtnMaterialThumbnailDownload
// FullName:  CDlgInfoDiffusionMaterial::OnBnClickedBtnMaterialThumbnailDownload
// Access:    public 
// Returns:   void
// Qualifier:下载素材缩略图
//************************************
void CDlgInfoDiffusionMaterial::OnBnClickedBtnMaterialThumbnailDownload()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    ...
    {
        char szLan1[512] = { 0 }; 
        g_StringLanType(szLan1, "正在下载或正在上传", "uploading or downloading");
        MessageBox(szLan1);
        return;
    }

    m_lUpdownloadHandle = NET_DVR_StartDownload(m_lUserID, NET_SDK_DOWNLOAD_PUBLISH_MATERIAL_THUMBNAIL, &m_dwDownMaterialIndex, sizeof(m_dwDownMaterialIndex), m_szSavePath.GetBuffer(0));
    if (m_lUpdownloadHandle == -1)
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_StartDownload NET_SDK_DOWNLOAD_PUBLISH_MATERIAL_THUMBNAIL");
        return;
    }
    g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_StartDownload NET_SDK_DOWNLOAD_PUBLISH_MATERIAL_THUMBNAIL");
