...
void CDlgStreamMediaCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_DMSIP, m_dmsIP);
    DDX_Text(pDX, IDC_EDIT_DMSPORT, m_dmsPort);
}
...
BOOL CDlgStreamMediaCfg::OnInitDialog()
{
    ...
    g_StringLanType(szLan, "dmsIP地址", "dmsIP");
    m_listCtrlStreamMediaCfg.InsertColumn(5, szLan, LVCFMT_LEFT, 100, -1);

    g_StringLanType(szLan, "dms端口", "dmsPort");
    m_listCtrlStreamMediaCfg.InsertColumn(6, szLan, LVCFMT_LEFT, 80, -1);
...
void CDlgStreamMediaCfg::OnBnClickedButtonSet()
{
    ...
    if (0 == iItemCount)
    {
        ...
        strncpy(pStreamMediaCfg->struDMSIP.sIpV4, m_dmsIP, 16);
        pStreamMediaCfg->wDMSPort = m_dmsPort;
    ...
    else
    {
        ...
        for (i = 0; i < iItemCount; i++)
        {
            ...
            strncpy(pStreamMediaCfg->struDMSIP.sIpV4, m_listCtrlStreamMediaCfg.GetItemText(i, 5), 16);
            pStreamMediaCfg->wDMSPort = _ttoi(m_listCtrlStreamMediaCfg.GetItemText(i, 6));
...
void CDlgStreamMediaCfg::OnBnClickedButtonGet()
{
    ...
    else
    {
        ...
        for (i = 0; i < iItemCount; i++)
        {
            ...
            strStatus.Format("%d", *pStatus);
            m_listCtrlStreamMediaCfg.SetItemText(i, 4, strStatus);
            m_listCtrlStreamMediaCfg.SetItemText(i, 5, pTempStreamMediaCfg->struDMSIP.sIpV4);
            
            CString strDMSPort;
            strDMSPort.Format("%d", pTempStreamMediaCfg->wDMSPort);
            m_listCtrlStreamMediaCfg.SetItemText(i, 6, strDMSPort);
...
void CDlgStreamMediaCfg::OnClickListStreamMediaCfg(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    else
    {
        ...
        m_dmsIP = m_listCtrlStreamMediaCfg.GetItemText(nItem, 5);
        CString strPortBuf = m_listCtrlStreamMediaCfg.GetItemText(nItem, 6);
        m_dmsPort = _ttoi(strPortBuf);
