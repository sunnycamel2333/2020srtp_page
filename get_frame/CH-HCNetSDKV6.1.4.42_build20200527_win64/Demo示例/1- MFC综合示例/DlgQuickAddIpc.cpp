...
void CDlgQuickAddIpc::OnBtnGetSadpinfo() 
{
    ...
    for (nSadpIndex = 0; nSadpIndex < m_struSadpInfoList.wSadpNum; nSadpIndex++)
    {
        ...
        if (ValidIPv6(m_struSadpInfoList.struSadpInfo[nSadpIndex].struIP.byIPv6))
        {
            char sIPv6[128] = {0};
            sprintf(sIPv6, "%s", m_struSadpInfoList.struSadpInfo[nSadpIndex].struIP.byIPv6);
            m_listSadpInfo.SetItemText(nSadpIndex, nIndex, sIPv6);
            nIndex++;
        }
        ...
        m_listSadpInfo.SetItemText(nSadpIndex, nIndex, (char *)m_struSadpInfoList.struSadpInfo[nSadpIndex].byDeviceModel);
        nIndex++;
    }
...
BOOL CDlgQuickAddIpc::OnInitDialog() 
{
    ...
    nIndex++;
    g_StringLanType(szLan, "IPV4地址", "IPV4 Address");
    ...
    g_StringLanType(szLan, "设备型号", "Device model");
    m_listSadpInfo.InsertColumn(nIndex, szLan, LVCFMT_LEFT, 80, -1);
    nIndex++;
...
BOOL CDlgQuickAddIpc::ValidIPv6(BYTE *ip)
{
