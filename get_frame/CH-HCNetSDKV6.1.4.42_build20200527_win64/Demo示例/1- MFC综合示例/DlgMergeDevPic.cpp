...
void CDlgMergeDevPic::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_PIC_OVERLAY_ENABLED, m_byPicOverlayEnabled);
    DDX_Control(pDX, IDC_COMBO_FLASH, m_comboFlash);
    DDX_Control(pDX, IDC_COMBO_SHOW, m_comboShow);
    DDX_Control(pDX, IDC_COMBO_TRANSLUCENT, m_comboTranslucent);
    DDX_Control(pDX, IDC_COMBO_PICTURE_TYPE, m_comboPicType);
    DDX_Text(pDX, IDC_EDT_OUTPUTNO, m_dwOutputNo);
    DDX_Text(pDX, IDC_EDT_WINDOWNO, m_dwWindowNo);
}
...
BEGIN_MESSAGE_MAP(CDlgMergeDevPic, CDialog)
    ...
    ON_WM_DESTROY()
    ON_BN_CLICKED(IDC_BTN_SETUP_LONGLINK, OnBnClickedBtnSetupLonglink)
    ON_BN_CLICKED(IDC_BTN_SEND_PIC_BY_BUF, OnBnClickedBtnSendPicByBuf)
    ON_BN_CLICKED(IDC_BTN_CLSOSE_LONGLINK, &CDlgMergeDevPic::OnBnClickedBtnClsoseLonglink)
END_MESSAGE_MAP()
...
void CDlgMergeDevPic::OnBnClickedBtnUploadByBuf()
{
    ...
    strncpy((char *)m_struPicCfg.sPicName, (LPCSTR)m_csPicName, sizeof(m_struPicCfg.sPicName));
    m_struPicCfg.sPicName[m_csPicName.GetLength()] = 0;
    m_struPicCfg.dwVideoWall = (1 << 24) | (m_dwOutputNo << 16) | m_dwWindowNo;
    //填充图片叠加参数
    m_struPicCfg.byOverlayEnabled = m_byPicOverlayEnabled;
    m_struPicCfg.byFlash = m_comboFlash.GetCurSel();
    m_struPicCfg.byTranslucent =  m_comboTranslucent.GetCurSel();
    m_struPicCfg.byShowEnabled =  m_comboShow.GetCurSel();
    m_struPicCfg.byPictureType = m_comboPicType.GetCurSel() + 1;
...
void CDlgMergeDevPic::OnDestroy()
{
    ...
    if (m_lUploadByBufLongHandle >= 0)
    {
        NET_DVR_UploadClose(m_lUploadByBufLongHandle);
        m_lUploadByBufLongHandle = -1;
    }
    CDialog::OnDestroy();
...
DWORD WINAPI CDlgMergeDevPic::PicUpLoadByBufLongThread(LPVOID pParam)
{
    CDlgMergeDevPic *pThis = (CDlgMergeDevPic*)pParam;
    ...
    while (bKeepLoop)
    {
        if (pThis->m_lUploadByBufLongHandle < 0)
        {
            bKeepLoop = FALSE;
        }
        iRet = NET_DVR_GetUploadState(pThis->m_lUploadByBufLongHandle, &iPos);
        if (iRet == -1)
        {
            continue;
        }
        ...
        szUpProgress.Format("%s...%d", csStr, iPos);
        pThis->SetDlgItemText(IDC_STATIC_PROGRESS, szUpProgress);
...
void CDlgMergeDevPic::OnBnClickedBtnSetupLonglink()
{
    // TODO:  在此添加控件通知处理程序代码
    if (m_lUploadByBufLongHandle <= -1)
    {
        m_lUploadByBufLongHandle = NET_DVR_UploadFile_V40(m_lUserID, UPLOAD_PIC_BY_BUF, NULL, 0, NULL, NULL, 0);
        if (m_lUploadByBufLongHandle == -1)
        {
            MessageBox("NET_DVR_UploadFile_V40 failed");
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_UploadFile_V40 failed");
        ...
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_UploadFile_V40 handle=[%d]", m_lUploadByBufLongHandle);

        //以下代码创建线程获取上传进度
        DWORD dwThreadID = 0;
        m_hPicUpLoadLongThread = CreateThread(NULL, 0, LPTHREAD_START_ROUTINE(PicUpLoadByBufLongThread), this, 0, &dwThreadID);
        if (m_hPicUpLoadLongThread == NULL)
        {
            char szLan[128] = { 0 };
            g_StringLanType(szLan, "开启获取图片上传进度线程失败!", "open get UpLoad picture progress thread Fail!");
            AfxMessageBox(szLan);
...
void CDlgMergeDevPic::OnBnClickedBtnSendPicByBuf()
{
    if (m_lUploadByBufLongHandle <= -1)
    {
        MessageBox("Please setup long link first!!");
        return;
    }
    if (m_bIsSending)
    {
        MessageBox("sending pic!!");
        return;
    }
    UpdateData(TRUE);
    memset(&m_struPicCfg, 0, sizeof(m_struPicCfg));
    ...
    strSendParamIn.dwSendAppendDataLen = sizeof(NET_DVR_PICTURECFG);
    strSendParamIn.byPicType = m_cmUploadSendPicType.GetCurSel() + 1;
    if (NET_DVR_UploadSend(m_lUploadByBufLongHandle, &strSendParamIn, NULL) < 0)
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_UploadSend error!");
        return;
    }
...
void CDlgMergeDevPic::OnBnClickedBtnClsoseLonglink()
{
    // TODO:  在此添加控件通知处理程序代码
    if (m_lUploadByBufLongHandle != -1)
    {
        BOOL iBool = NET_DVR_UploadClose(m_lUploadByBufLongHandle);
        if (iBool)
        {
            m_bIsSending = FALSE;
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_CloseUploadHandle success!");
            m_lUploadByBufLongHandle = -1;
        ...
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_CloseUploadHandle error!");
            return;
        }
