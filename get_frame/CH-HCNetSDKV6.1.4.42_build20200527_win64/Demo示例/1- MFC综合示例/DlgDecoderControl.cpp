...
void CDlgDecoderControl::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Text(pDX, IDC_EDIT2_VOLUME, m_dwVolume);
    DDX_Check(pDX, IDC_CHK_ENABLE_AUDIO, m_bEnableAudio);
}
...
void CDlgDecoderControl::OnButDecodeGet() 
{
    ...
    m_bEnableAudio = struDecChan.byEnableAudio;
    m_dwVolume = struDecChan.byVolume;
	UpdateData(FALSE);
}
...
void CDlgDecoderControl::OnButDecodeSet() 
{
    ...
    struDecChan.byEnableAudio = m_bEnableAudio;
    struDecChan.byVolume = m_dwVolume;
