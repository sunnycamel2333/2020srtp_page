...
BEGIN_MESSAGE_MAP(CDlgProductAcs, CDialog)
    ...
    ON_BN_CLICKED(IDC_BTN_WORK_MODE, &CDlgProductAcs::OnBnClickedBtnWorkMode)
END_MESSAGE_MAP()
...
void CDlgProductAcs::OnButAcAlarmUser() 
{
    ...
    NET_DVR_GetDeviceAbility(m_lUserID, ALARMHOST_ABILITY, NULL, 0, (char*)&(struAlarmHostAbility), sizeof(struAlarmHostAbility));
    dlg.m_dwNetUserNum = ((struAlarmHostAbility.wNetUserNum == 0) ? 1 : struAlarmHostAbility.wNetUserNum);
    dlg.DoModal();
}
...
void CDlgProductAcs::OnBnClickedBtnWorkMode()
{
    // TODO:  在此添加控件通知处理程序代码
    CDlgAcsWorkModeCfg dlg;
    dlg.m_lUserID = m_lUserID;
    dlg.m_iDeviceIndex = m_iDeviceIndex;
    dlg.DoModal();
}
