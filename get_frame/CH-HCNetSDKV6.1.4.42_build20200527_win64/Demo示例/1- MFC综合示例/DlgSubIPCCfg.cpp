...
BEGIN_MESSAGE_MAP(CDlgSubIPCCfg, CDialog)
    ...
    ON_BN_CLICKED(IDC_BUTTON1, &CDlgSubIPCCfg::OnBnClickedButton1)
    ON_BN_CLICKED(IDC_BTN_FILE_UPLOAD_DOWNLOAD, &CDlgSubIPCCfg::OnBnClickedBtnFileUploadDownload)
END_MESSAGE_MAP()
...
void CDlgSubIPCCfg::OnBnClickedButton1()
{
    CDlgThermometry dlg;
    dlg.m_lChannel = m_lChannel;
    dlg.m_lUserID = m_lUserID;
...
void CDlgSubIPCCfg::OnBnClickedBtnFileUploadDownload()
{
    // TODO:  在此添加控件通知处理程序代码
    CDlgFileUploadDownload dlg;
    dlg.m_lChannel = m_lChannel;
    dlg.m_lUserID = m_lUserID;
    dlg.m_iDevIndex = m_lDeviceIndex;
    dlg.DoModal();
