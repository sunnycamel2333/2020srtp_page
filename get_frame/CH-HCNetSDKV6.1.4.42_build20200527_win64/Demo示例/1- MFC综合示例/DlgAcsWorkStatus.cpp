...
void CDlgAcsWorkStatus::UpdateControlData()
{
    ...
    for (i = 0; i < MAX_CARD_READER_NUM_512; i++)
    {
        ...
        sprintf(szLan, "%d", m_struAcsWorkStatus.byCardReaderVerifyMode[i]);
        ...
        else if (m_struAcsWorkStatus.byCardReaderVerifyMode[i] == 25)
        {
            g_StringLanType(szLan, "刷卡或人脸", "card or face or password");
        }
        else if (m_struAcsWorkStatus.byCardReaderVerifyMode[i] == 26)
        {
            g_StringLanType(szLan, "刷卡或人脸或指纹", "card or face or password");
        }
        else if (m_struAcsWorkStatus.byCardReaderVerifyMode[i] == 27)
        {
            g_StringLanType(szLan, "刷卡或指纹或密码", "card or fingerprint or password");
        }
