...
void CDlgPictureUpload::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMBO_PICTURE_USE_TYPE, m_pictureUserType);
    DDX_Text(pDX, IDC_EDIT_PICTURE_ID, m_pictureId);
}
...
BOOL CDlgPictureUpload::OnInitDialog() 
{
    ...
    char szLan[128] = { 0 };
    g_StringLanType(szLan, "屏保", "screen protect");
    m_pictureUserType.InsertString(0, szLan);
    g_StringLanType(szLan, "开机logo", "open logo");
    m_pictureUserType.InsertString(1, szLan);
    m_pictureUserType.SetCurSel(0);
...
void CDlgPictureUpload::OnBtnPicUpload() 
{
    ...
    if (m_bUpLoading == FALSE)
    {
        ...
        m_struPicInfo.byUseType = m_pictureUserType.GetCurSel();
        m_struPicInfo.dwPicMangeNo = m_pictureId;
