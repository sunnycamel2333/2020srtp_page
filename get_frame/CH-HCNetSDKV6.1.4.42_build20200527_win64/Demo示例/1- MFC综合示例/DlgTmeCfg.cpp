...
//    m_csEntranceVehCardXMLBlock = xmlBase.GetChildren().c_str();
    ...
    //转为UTF-8
#ifdef DEMO_LAN_CN
    string str_utf = GB2UTF(m_csEntranceVehCardXMLBlock.GetBuffer(0));
    struCfg.lpInBuffer = (void*)str_utf.c_str();
    struCfg.dwInSize = str_utf.length();
#endif // DEMO_LAN_CN
    memset(m_szStatusBuf, 0, ISAPI_STATUS_LEN);
    struCfg.lpStatusBuffer = m_szStatusBuf;
    struCfg.dwStatusSize = ISAPI_STATUS_LEN;
...
void CDlgTmeCfg::OnBtnGetEntranceVehcardCfg() 
{
   ...
   // m_csEntranceVehCardXMLBlock.Format(_T("%s"), pGetXmlBuf);
    ...
    //转为GB312
#ifdef DEMO_LAN_CN
        string str_gb2312 = UTF2GB(m_csEntranceVehCardXMLBlock);
        m_csEntranceVehCardXMLBlock = str_gb2312.c_str();
#endif // DEMO_LAN_CN
