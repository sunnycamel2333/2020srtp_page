...
void CDlgIPCPlateRecog::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_FILTRATION, m_bFiltration);
}
...
BEGIN_MESSAGE_MAP(CDlgIPCPlateRecog, CDialog)
    ...
    ON_BN_CLICKED(IDC_BUTTON_SET_FILTRATION, &CDlgIPCPlateRecog::OnBnClickedButtonSetFiltration)
    ON_BN_CLICKED(IDC_BUTTON_GET_FILTRATION, &CDlgIPCPlateRecog::OnBnClickedButtonGetFiltration)
END_MESSAGE_MAP()
...
void CDlgIPCPlateRecog::OnBnClickedButtonSetFiltration()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    DWORD dwInputLen = ISAPI_DATA_LEN;
    char *szInParamBuf = new char[dwInputLen];
    char *szOutParamBuf = new char[ISAPI_DATA_LEN];
    char szUrl[256] = { 0 };
    sprintf(szUrl, "PUT /ISAPI/Traffic/channels/%d/licensePlate/filtration?format=json\r\n", m_lChannel);

    cJSON* pRoot = cJSON_CreateObject();
    cJSON* pFiltration = cJSON_CreateObject();
    cJSON* pEnabled = cJSON_CreateObject();
    pEnabled->type = m_bFiltration;
    cJSON_AddItemToObject(pRoot, "Filtration", pFiltration);
    cJSON_AddItemToObject(pFiltration, "enabled", pEnabled);

    szInParamBuf = cJSON_Print(pRoot);
    NET_DVR_XML_CONFIG_INPUT struXMLCfgInput = { 0 };
    struXMLCfgInput.dwSize = sizeof(struXMLCfgInput);
    struXMLCfgInput.lpRequestUrl = szUrl;
    struXMLCfgInput.dwRequestUrlLen = strlen(szUrl);
    struXMLCfgInput.lpInBuffer = szInParamBuf;
    struXMLCfgInput.dwInBufferSize = dwInputLen;

    NET_DVR_XML_CONFIG_OUTPUT struXMLCfgOutput = { 0 };
    struXMLCfgOutput.dwSize = sizeof(struXMLCfgOutput);
    struXMLCfgOutput.lpOutBuffer = (char*)szOutParamBuf;
    struXMLCfgOutput.dwOutBufferSize = ISAPI_DATA_LEN;
    struXMLCfgOutput.lpStatusBuffer = NULL;
    struXMLCfgOutput.dwStatusSize = 0;

    if (NET_DVR_STDXMLConfig(m_lServerID, &struXMLCfgInput, &struXMLCfgOutput))
    {
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, szUrl);
    }
    else
...
void CDlgIPCPlateRecog::OnBnClickedButtonGetFiltration()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    NET_DVR_XML_CONFIG_INPUT    struInput = { 0 };
    ...
    struOuput.dwSize = sizeof(struOuput);
    char szUrl[256] = { 0 };
    _snprintf(szUrl, sizeof(szUrl), "GET /ISAPI/Traffic/channels/%d/licensePlate/filtration?format=json\r\n", m_lChannel);
    ...
    cJSON* pFiltration = pRoot->child;
    if (NULL != pFiltration && TRUE == pFiltration->child->type)
    {
        m_bFiltration = TRUE;
    }
    else
