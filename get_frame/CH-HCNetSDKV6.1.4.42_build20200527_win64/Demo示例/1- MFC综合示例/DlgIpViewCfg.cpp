...
void CDlgIpViewCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_CALLED_USERNAME, m_csCalledUsedName);
}
...
void CDlgIpViewCfg::OnBtnGetSipCfg()
{
    ...
    byte temp[65] = { 0 };
    memcpy(temp, m_struSipCfg.bySIPServerDomain, 64);
    m_csDomainName.Format("%s", temp);
    ...
    m_csCalledUsedName.Format("%s", m_struSipCfg.byCalledTargetName);
    UpdateData(FALSE);
...
void CDlgIpViewCfg::OnBtnSetSipCfg()
{
    ...
    if (m_csCalledUsedName.GetLength() > 32)
    {
        g_StringLanType(szLan, "呼叫用户名不能超过32字节长！", "calledTargetUserName length could not be over 32!");
        AfxMessageBox(szLan);
        return;
    }
    sprintf((char *)m_struSipCfg.byCalledTargetName, "%s", m_csCalledUsedName);
