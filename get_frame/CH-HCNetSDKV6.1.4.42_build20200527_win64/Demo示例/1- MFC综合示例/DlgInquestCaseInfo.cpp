...
void CDlgInquestCaseInfo::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Text(pDX, IDC_CASE_CUSTOM_INFO4, m_sCustomInfo4);
    DDX_Text(pDX, IDC_CASE_CUSTOM_INFO5, m_sCustomInfo5);
}
...
void CDlgInquestCaseInfo::OnBtnSet() 
{
    ...
    sprintf(sCaseName, "%s", m_sCaseName.GetBuffer());
    ...
    sprintf(sCaseNo, "%s", m_sCaseNo.GetBuffer());
    ...
    sprintf(sCustomInfo1, "%s", m_sCustomInfo1.GetBuffer());
    ...
    sprintf(sCustomInfo2, "%s", m_sCustomInfo2.GetBuffer());
    ...
    sprintf(sCustomInfo3, "%s", m_sCustomInfo3.GetBuffer());
    ...
    if (m_sCustomInfo4.GetLength() > CUSTOM_INFO_LEN)
    {
        AfxMessageBox("自定义信息4超过最大长度");
        return;
    }
    char sCustomInfo4[CUSTOM_INFO_LEN] = { 0 };
    sprintf(sCustomInfo4, "%s", m_sCustomInfo4.GetBuffer());
    memcpy(m_struInqCaseInfo.byCustomInfo4, sCustomInfo4, CUSTOM_INFO_LEN);

    if (m_sCustomInfo5.GetLength() > CUSTOM_INFO_LEN)
    {
        AfxMessageBox("自定义信息5超过最大长度");
        return;
    }
    char sCustomInfo5[CUSTOM_INFO_LEN] = { 0 };
    sprintf(sCustomInfo5, "%s", m_sCustomInfo5.GetBuffer());
    memcpy(m_struInqCaseInfo.byCustomInfo5, sCustomInfo5, CUSTOM_INFO_LEN);
...
void CDlgInquestCaseInfo::OnBtnGet() 
{
    ...
    char sCaseName[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCaseName, m_struInqCaseInfo.sCaseName, CUSTOM_INFO_LEN);
    m_sCaseName.Format("%s", sCaseName);

    char sCaseNo[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCaseNo, m_struInqCaseInfo.sCaseNo, CUSTOM_INFO_LEN);
    m_sCaseNo.Format("%s", sCaseNo);

    char sCustomInfo1[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCustomInfo1, m_struInqCaseInfo.sCustomInfo1, CUSTOM_INFO_LEN);
    m_sCustomInfo1.Format("%s", sCustomInfo1);

    char sCustomInfo2[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCustomInfo2, m_struInqCaseInfo.sCustomInfo2, CUSTOM_INFO_LEN);
    m_sCustomInfo2.Format("%s", sCustomInfo2);

    char sCustomInfo3[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCustomInfo3, m_struInqCaseInfo.sCustomInfo3, CUSTOM_INFO_LEN);
    m_sCustomInfo3.Format("%s", sCustomInfo3);
    ...
    char sCustomInfo4[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCustomInfo4, m_struInqCaseInfo.byCustomInfo4, CUSTOM_INFO_LEN);
    m_sCustomInfo4.Format("%s", sCustomInfo4);

    char sCustomInfo5[INQUEST_CASE_NAME_LEN + 1] = { 0 };
    memcpy(sCustomInfo5, m_struInqCaseInfo.byCustomInfo5, CUSTOM_INFO_LEN);
    m_sCustomInfo5.Format("%s", sCustomInfo5);
