...
void CDlgCertInfo::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
    DDX_Check(pDX, IDC_CHK_ADDITION, m_bAddition);
    DDX_Text(pDX, IDC_EDIT_CUSTOM_ID, m_csCustomID);
    DDX_Control(pDX, IDC_COMBO_CERTIFICATE_MODE, m_combCertificateMode);
    DDX_Control(pDX, IDC_COMBO_PRIVATE_KEY_MODE, m_combPrivateKeyMode);
...
BEGIN_MESSAGE_MAP(CDlgCertInfo, CDialog)
	...
	ON_WM_TIMER()
	//}}AFX_MSG_MAP
    ON_BN_CLICKED(IDC_CHK_ADDITION, &CDlgCertInfo::OnBnClickedChkAddition)
END_MESSAGE_MAP()
...
BOOL CDlgCertInfo::OnInitDialog() 
{
    ...
    m_combCertificateMode.SetCurSel(0);
    m_combPrivateKeyMode.SetCurSel(0);
    GetDlgItem(IDC_EDIT_CUSTOM_ID)->EnableWindow(FALSE);
    GetDlgItem(IDC_COMBO_CERTIFICATE_MODE)->EnableWindow(FALSE);
    GetDlgItem(IDC_COMBO_PRIVATE_KEY_MODE)->EnableWindow(FALSE);
    GetDlgItem(IDC_EDIT_PASSWORD)->EnableWindow(FALSE);
    GetDlgItem(IDC_CUSTOM_ID)->EnableWindow(FALSE);
    GetDlgItem(IDC_CLIENT_SERVER_TYPE)->EnableWindow(FALSE);
    GetDlgItem(IDC_CERT_PRIVATE_TYPE)->EnableWindow(FALSE);
...
void CDlgCertInfo::OnButtonCertUpload() 
{
	...
	NET_DVR_CERT_PARAM struCertParam;
	memset(&struCertParam, 0, sizeof(NET_DVR_CERT_PARAM));
    NET_DVR_CERT_ADDITION_PARAM struCertAdditionParam;
    memset(&struCertAdditionParam, 0, sizeof(NET_DVR_CERT_ADDITION_PARAM));
	
	struCertParam.dwSize = sizeof(NET_DVR_CERT_PARAM);
	struCertParam.wCertFunc = m_wCertFunc;
	struCertParam.wCertType = m_wCertType;
	struCertParam.byFileType = m_byCertFileType;
    struCertParam.byAddition = m_bAddition;
    if (struCertParam.byAddition)
    {
        struCertAdditionParam.dwSize = sizeof(NET_DVR_CERT_ADDITION_PARAM);
        memcpy(struCertAdditionParam.csCustomID, m_csCustomID, 64);
        struCertAdditionParam.byCertificateMode = m_combCertificateMode.GetCurSel();
        if (struCertAdditionParam.byCertificateMode)
        {
            struCertAdditionParam.byPrivateKeyMode = m_combPrivateKeyMode.GetCurSel();
        }
        memcpy(struCertAdditionParam.byPassword, m_csPassword, 64);
        struCertParam.pStruAdditionParam = &struCertAdditionParam;
    }
...
#endif
{
	...
	else if (nIDEvent == UPLOAD_TIMER)
	{
        ...
        {
            g_StringLanType(szLan, "证书已存在", "certificate already exist");
            m_strUploadStatus.Format(szLan);
        }
        g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_GetUploadState[%d][%s]", state, szLan);
...
void CDlgCertInfo::OnBnClickedChkAddition()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    if (m_bAddition)
    {
        GetDlgItem(IDC_EDIT_CUSTOM_ID)->EnableWindow(TRUE);
        GetDlgItem(IDC_EDIT_CUSTOM_ID)->EnableWindow(TRUE);
        GetDlgItem(IDC_COMBO_CERTIFICATE_MODE)->EnableWindow(TRUE);
        GetDlgItem(IDC_COMBO_PRIVATE_KEY_MODE)->EnableWindow(TRUE);
        GetDlgItem(IDC_EDIT_PASSWORD)->EnableWindow(TRUE);
        GetDlgItem(IDC_CUSTOM_ID)->EnableWindow(TRUE);
        GetDlgItem(IDC_CLIENT_SERVER_TYPE)->EnableWindow(TRUE);
        GetDlgItem(IDC_CERT_PRIVATE_TYPE)->EnableWindow(TRUE);
